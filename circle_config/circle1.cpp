...
static bool is_circle(Point p, int radius, Circle_runtime_param &circle_runtime_param, double *score, int sensi)
{
	...
	// 含义就是角度偏差容忍度从5度到15度，0.012是等间隔的值
	double angle_bias = 0.085 + 0.012 * (sensi - 1);
	...
	for (int i = -180; i < 180; i += 1) {
		...
		if (fabs(angle - radial_direction) < angle_bias || fabs(angle - radial_direction1) < angle_bias) {
			count++;
		}
	}
	*score = 1.0 * count / 360;
...
static void prepare_data(Circle_runtime_param &circle_runtime_param, int MAG_THRESH)
{
...
static void hough(Circle_runtime_param &circle_runtime_param, int sensi, double threshold, int minRadius, int maxRadius, double distance, std::vector<Circle> &circles, int *num_circles)
{
	/*
	// 强度阈值的设定
	// 设定阈值范围

	int MAG_THRESHOLD[16] = {30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180};
	int MAG_THRESH = MAG_THRESHOLD[sensi-1];
	*/
	...
	int regionx = 0;
	int regiony = 0;
	int region_height = circle_runtime_param.img_gray.rows;
	int region_width = circle_runtime_param.img_gray.cols;
	...
	for (int i = 0; i < number_of_best_cirles; i++) {
		...
		if (is_circle(Point(xCoord, yCoord), radius, circle_runtime_param, &score, sensi)) {
			Circle circle;
...
int circle_detection_config(const UINT8 *yuv, std::vector<Result_Circle>& circles1, int sensi)
{
	...
	prepare_data(circle_runtime_param, MAG_THRESH);
